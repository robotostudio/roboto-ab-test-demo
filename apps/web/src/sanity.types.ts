...
// Query: *[_type == "page" && defined(slug.current) && !seoNoIndex]{  "slug":slug.current,}
...
export type GetAllMainPageTranslationsQueryResult = Array<null>;

// Variable: getBlogIndexDataQuery
// Query: {    "seo":*[_type == "blogIndex"][0]{        ...,    },    "blogs":*[_type == "blog"]{      _id,      "title":coalesce(cardTitle,title),"description":coalesce(cardDescription,description),"image":coalesce(cardImage,image),      "slug":slug.current    }}
...
export type GetAllBlogIndexTranslationsQueryResult = Array<null>;

// Variable: getAllBlogsPathsQuery
// Query: *[_type == "blog" && defined(slug.current) && !seoNoIndex]{  "slug":slug.current,}
...
// Query: *[_type == "blog" && slug.current == $slug][0]{    ...,    defined(richText)=>{  richText[]{    ...,     defined(markDefs)=>{  markDefs[]{    ...,    defined(customLink)=>{  customLink{    openInNewTab,    "href": select(type == "internal"=>internal->slug.current, type == "external" => external,"#"),  }}     }}     }}  }  
...
// Query: *[_type == "page" && slug.current == $slug ][0]{    _id,    _type,    title,    "slug":slug.current,    defined(pageBuilder)=>{  pageBuilder[]{    ...,    _type,    defined(buttons)=>{  buttons[]{    ...,    defined(url)=>{  url{    openInNewTab,    "href": select(type == "internal"=>internal->slug.current, type == "external" => external,"#"),  }},    defined(icon)=>{  icon{    svg  }}  }},    defined(richText)=>{  richText[]{    ...,     defined(markDefs)=>{  markDefs[]{    ...,    defined(customLink)=>{  customLink{    openInNewTab,    "href": select(type == "internal"=>internal->slug.current, type == "external" => external,"#"),  }}     }}     }},    defined(form)=>{  form->{    ...,  }}  }}}
...
// Variable: abTestQuery
// Query: *[_type == "abTest" && enabled]{    _id,    "variants":variants[]->slug.current}
export type AbTestQueryResult = Array<{
  _id: string;
  variants: Array<string | null> | null;
}>;

// Variable: abTestPageQuery
// Query: *[_type == "abTest" && enabled && $slug in variants[]->slug.current][0]{  "slugs": variants[]->slug.current}
export type AbTestPageQueryResult = {
